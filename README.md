...
- **Исполнитель** — пишет код по задаче. Движок выбирается конфигом
  ...
  исполнителя надо видеть в бюджете прогона, **ZCode** (клиент Z.AI,
  `zcode`) — третий CLI, та же роль, другая подписка. Ревьюер на ZCode
  не переводится.
...
(или `zcode --prompt`, если `executor_engine = "zcode"`).
