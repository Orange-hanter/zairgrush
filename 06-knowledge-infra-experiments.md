...
1. **Замечания** — `experiments/findings.jsonl`, одна строка на наблюдение:
   ...
   Канонический набор `kind` и отображение хвоста, накопленного практикой,
   — в [13-findings-taxonomy.md](13-findings-taxonomy.md) §4; новые
   значения не вводятся без записи там.
...
- **Статус: вариант B реализован** (2026-09-13, NXT-016, findings
  E2/task-cli): `swarm task add/split/close/list/check` —
  `tools/swarm/swarm/clitask.py`, 189 строк на stdlib, smoke
  `test_task_cli.py` (13 тестов). Схему §4.3 проверяет одна функция
  `task_errors` на все глаголы; формат id не копирован, а взят из
  `planner._ID_RE`. «Запрет ручного редактирования» — не замок (плоский
  файл не запереть), а след целостности: `task check` сверяет отпечаток
  tasks.json с последним объявленным в журнале, та же механика §6.1.
  Живой smoke на синтетическом стенде: add со схемой и idea проходят,
  add без paths/acceptance отвергается, split расщепляет с наследованием
  границ, ручная правка файла ловится `check` (exit 2). Метрика решения
  (невалидные задачи за прогон, время оформления человеком) НЕ снята —
  для неё нужен настоящий прогон; эксперимент открыт.
...
- **C (SCIP blast radius)**: индекс `rust-analyzer scip .` (обновление
  ...
  `paths`/`deps`.
- **Статус C (prep, 2026-09-15, WAV-006)**: бесплатная половина
  сделана, замер (REV-008) зарегистрирован, не проведён. Стенд
  пересажен с мёртвого tenacity на zeus-pilot
  (`run_blast.py --stand`, gate `cargo test --workspace`, `--dry-run`
  проверяет весь контур без LLM); blast-данные — `scip_rust.py`
  (точные ссылки компиляторного фронтенда, не ast/ctags догадки);
  3 канарейки + 2 чистых контроля валидированы прогоном gate
  (поломка реальна, тесты зелёные; 2 кандидата отбракованы — ловят
  интеграционные тесты, невидимые SCIP-счётчику). Флаг `review_blast`
  (default off) в оркестраторе: `blastnote.py` + `_blast_note_for` в
  reviewer (кэш индекса по отпечатку дерева, fail-open, журнал
  `blast_note`), блок сразу за диффом в `review_prompt_parts`; чистый
  дифф не меняет промпт ни на байт. Платная фаза: 10 вызовов,
  кап $20, команды в `experiments/reviewarm/rev008/prereg.json`.
...
- **Статус: вариант B реализован, бесплатная половина замера снята**
  (2026-09-14, NXT-017, findings E5/docmap). `docmap.toml` в корне репо
  (40 записей «glob кода → `файл.md#якорь`», якорь — подстрока
  заголовка markdown, stdlib tomllib), детектор
  `tools/swarm/swarm/docmap.py` (~140 строк, чистый stdlib), CLI
  `swarm docmap [--changed …]`; хелпер A (docctx/cod-doc) не тронут —
  варианты живут рядом. Карта самопроверяема механически: мёртвый glob,
  нет документа, нет якоря — warnings; жизнеспособность карты держит
  тест в гейте. Реплей 85 коммитов кода из истории
  (`experiments/tools/e5-docdrift-replay.py`, сырьё
  `experiments/e5/drift-replay.jsonl`): земля «док правлен в том же
  коммите, что и код» — B назвал 46/58 против 23/58 у A-эвристики
  (упоминание имени файла); земля «док правлен в следующих 10
  коммитах» — 85/131 против 33/131; ложных подозрений поровну (32
  против 31). Оговорки: карта написана постфактум (замер отвечает
  «находит ли», а не «поддерживали бы»); вторая земля шумная —
  документы правятся и по внекодовым причинам; три модуля
  (agents_types, loop_types, modlock) сознательно вне карты. Токены
  хелпера и «LLM формулирует правку» — платная половина, отложена до
  живого прогона; механические половины обоих вариантов — 0 токенов по
  построению. Эксперимент открыт.
...
| **E3-C prep** (blast radius ревьюеру) | prep done 2026-09-15, замер пререгистрирован | experiments/reviewarm/rev008/prereg.json, experiments/repomap/run_blast.py | Стенд zeus-pilot (Rust) + `scip_rust` (SCIP из rust-analyzer). 3 канарейки, ломающие непокрытого вызывающего при зелёном gate (валидация — прогоном `cargo test`), +2 чистых контроля; 2 кандидата отбракованы (интеграционные тесты невидимы SCIP-счётчику). Флаг `review_blast` в оркестраторе (default off, гейт зелёный). Платный замер REV-008: 10 вызовов, кап $20 |
...
семантическое ослабление c5. Открытые: **E3 вариант C — prep завершён
(2026-09-15, WAV-006)**: стенд zeus-pilot + scip_rust, 3 канарейки и 2
контроля валидированы прогоном gate, флаг `review_blast` в оркестраторе
за пререгом REV-008 (default off), платная фаза — 10 вызовов под капом
$20 (команды в rev008/prereg.json), **E2 — вариант B (task-CLI)
реализован и прошёл smoke (2026-09-13)**, решающий замер ждёт настоящего
прогона, **E5 — вариант B (docmap) реализован и замерен офлайн
(2026-09-14)**: реплей по истории даёт recall вдвое выше A-эвристики при
том же шуме, платная половина (токены, формулировка правки) отложена,
E7 — в очереди; E6 ожил
в составе E9;
...
- **E3-C: prep варианта C завершён без платных вызовов** (WAV-006,
  findings E3/blast-prep, пререг REV-008): стенд blast-радиуса
  пересажен с мёртвого tenacity на живой zeus-pilot
  (`run_blast.py --stand`, gate `cargo test --workspace`, `--dry-run`
  проверяет контур бесплатно); blast-данные Rust — `scip_rust.py`
  (SCIP из rust-analyzer, ADR-007). 3 канарейки (set_once, pick,
  informations — контракт хелпера ломает непокрытых вызывающих,
  gate зелёный) + 2 чистых контроля; 2 кандидата отбракованы
  прогоном gate (severity_label, meaningful — ловят интеграционные
  тесты, невидимые SCIP-счётчику ссылок). В оркестраторе флаг
  `review_blast` (default off): `blastnote.py` (символы из диффа),
  `_blast_note_for` в reviewer (кэш индекса по отпечатку дерева,
  fail-open, журнал `blast_note`), блок сразу за диффом в
  `review_prompt_parts`; чистый дифф — ноль байт разницы.
  Платная фаза REV-008: 10 вызовов, кап $20, правило решения —
  в `experiments/reviewarm/rev008/prereg.json`.
...
- §3: указатель на канонический набор `kind` в 13-findings-taxonomy.md §4
  (WAV-009) — объявленные здесь три значения практика расширила до
  пятнадцати; таксономия фиксирует шесть канонических и отображение
  хвоста. Бюджетная дисциплина волн (пул $60, политика отказов,
  NXT-026) записана в 08-operators-guide.md, «Бюджет волны».
...
- **E5: вариант B (docmap) реализован, бесплатная половина замера
  снята** (NXT-017, findings E5/docmap). `docmap.toml` (40 записей,
  stdlib tomllib), детектор `swarm/docmap.py`, CLI `swarm docmap`;
  docctx не тронут. Реплей по git-истории (85 коммитов кода): recall
  B 46/58 и 85/131 на двух классах земли против 23/58 и 33/131 у
  A-эвристики, шум одинаковый. Платная половина (токены хелпера,
  формулировка правки LLM) отложена; эксперимент открыт.

### v0.33 (2026-09-13)

- E2: вариант B (task-CLI) реализован — `swarm task add/split/close/
  list/check` в tools/swarm (clitask.py, 189 строк, один валидатор
  схемы §4.3 на все глаголы, формат id из planner._ID_RE), smoke
  test_task_cli.py (13 тестов). «Запрет ручной правки» — след
  целостности (отпечаток против журнала), не замок. Метрика решения не
  снята: нужен настоящий прогон, эксперимент открыт.
