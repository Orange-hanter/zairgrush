...
2a. **Фоновый замер — второй способ мерить, и с 2026-08-24 основной для
   ...
   качеством наблюдений. Поэтому `experiments/tools/ambient-report.py`
...
- **Статус: зарегистрирован, не реализован.** Сознательно: правило
  программы — дублирующий вариант и критерий решения ДО постройки, а
  дорогих ролей в петле уже четыре. Добавить пятую и шестую «потому что
  звучит верно» — ровно тот ход, против которого и заводился замер
  выживаемости мутантов. Первым строится КОНТРгипотеза, она дешевле и
  падает быстрее.
...
гигиеной линтера, а не памятью. Плечо ревьюера прогона ещё ждёт; **E12 — оба реплей-стенда
