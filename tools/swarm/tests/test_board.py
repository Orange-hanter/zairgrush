...
class TestCollectFacts(unittest.TestCase):
    ...
    def test_cost_without_task_counts_in_total_only(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = make_root(
                ...
                metrics=[{"task": "t1", "cost_usd": 1.0}, {"cost_usd": 0.5}],
            )
            board = bd.collect(root)
    ...
    def test_suppressed_grouped_by_task(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            board = bd.collect(root)
        self.assertEqual(
            board["tasks"][0]["_suppressed"], [{"policy": "шум", "issue": "мелочь"}]
        )
...
class TestRunLevelIsVisible(unittest.TestCase):
    ...
    def test_plan_failed_reaches_run_level(self):
        ...
        self.assertNotIn(
            "state_written", kinds, "бухгалтерия хоронит под собой редкие события"
        )
...
class TestVerdictOrderIsNumeric(unittest.TestCase):
    ...
    def test_round_ten_sorts_after_round_two(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rounds = [v["round"] for v in bd._verdicts(root / ".swarm", "aaaa")]
...
class TestRenderSafety(unittest.TestCase):
    ...
    def test_title_does_not_break_out_of_json_payload(self):
        page = self.render_one(
            id="t1", status="done", title="x</script><script>alert(1)</script>"
        )
...
class TestBuild(unittest.TestCase):
    def test_writes_board_html_next_to_state(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = make_root(
                tmp, tasks=[{"id": "t1", "status": "done"}], goal="проверка"
            )
...
class TestLiveBoard(unittest.TestCase):
    ...
    def test_loop_rewrites_the_page(self):
        self.lp.Loop(self.state, {"live_board": True}, None).refresh_board()
    ...
    def test_failure_to_build_does_not_stop_the_loop(self):
        ...
        loop = self.lp.Loop(self.state, {"live_board": True}, None)
...
class TestRunFacts(unittest.TestCase):
    ...
    def test_money_is_split_by_role(self):
        ...
        self.assertEqual(
            round(sum(p["cost"] for p in phases.values()), 2), board["total"]
        )
