...
# Слово короче четырёх букв стеммер всё равно сотрёт в стоп-слово или в
# общий корень; в запросе оно только удлиняет разбор. Потолок в 12 слов —
# от длинных спецификаций: ts_rank считает по всем термам, и хвост из
# сотни слов ранжирование не улучшает, а замедляет.
_FTS_MIN_WORD = 4
_FTS_MAX_WORDS = 12
...
def fts_query(text: str) -> str:
    """Запрос для `websearch_to_tsquery` из произвольного текста.

    ЗАЧЕМ ОН ВООБЩЕ ЕСТЬ. `websearch_to_tsquery` соединяет слова через
    И. Пока сюда уезжала спецификация задачи целиком, условие требовало,
    чтобы в одном уроке встретились ВСЕ её слова, — и не выполнялось
    никогда. Замер на пилоте (2026-08-23): по всем настоящим запросам
    FTS отдавал РОВНО НОЛЬ строк, а все хиты приходили от вектора. То
    есть «FTS первым, вектор — сеть охвата» из дизайна на деле было
    «только вектор», и E6 едва не получил ответ в пользу эмбеддингов по
    итогам сравнения, которого не было.

    Разделитель `or` — часть websearch-синтаксиса, поэтому данные
    по-прежнему едут параметром, а не склейкой SQL. После правки те же
    запросы дают 3–5 строк вместо нуля.
    """
    words = [w.lower() for w in _FTS_WORD.findall(text or "")
             if len(w) >= _FTS_MIN_WORD]
    uniq = list(dict.fromkeys(words))[:_FTS_MAX_WORDS]
    return " or ".join(uniq)
...
def search_fts(config: dict[str, Any], repo: str, query: str,
    ...
    query = fts_query(query) or query
