...
class Symbol:
    __slots__ = ("id", "name", "module", "file", "line", "sig", "doc", "kind")
...
class Reference:
    __slots__ = ("symbol_id", "file", "line", "from_symbol", "how")
...
class Index:
    ...
    def __init__(self, root):
        ...
        self._by_name = {}           # короткое имя -> [id] (для отчётов)
        self._build()
    ...
    def _collect_defs(self, path, tree):
    ...
    def _collect_refs(self, path, tree):
        module = _module_name(path, self.root)
        rel = path.relative_to(self.root).as_posix()
        imports = self._imports(tree, module)

        def enclosing(prefix, node):
            return prefix
    ...
    def _record_call(self, call, from_symbol, rel, module, imports):
        fn = call.func
    ...
    def rank(self, damping=0.85, rounds=20):
    ...
    def project_map(self, budget=25, include_tests=False):
        ...
        for f in sorted(by_file):
            ...
            for sid in sorted(by_file[f], key=lambda s: self.symbols[s].line):
                ...
                used = len([r for r in self.callers(sid) if not r.file.startswith("tests/")])
                doc = f" — {sym.doc}" if sym.doc else ""
                mark = f"  [зовут: {used}]" if used else ""
                out.append(f"  {sym.sig}{doc}{mark}")
        return "\n".join(out)

    def impact(self, symbol_ids):
