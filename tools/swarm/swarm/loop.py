...
class Loop:
    ...
    def _run_task(self, task: dict[str, Any]) -> str:
        ...
        # исполнителем НЕ проверяется намеренно: тесты кодируют спеку, а
        # не обязаны быть красными — у задачи, которая СОХРАНЯЕТ поведение
        # (кэш, ускорение, рефакторинг), охранные тесты зелёные и до, и
        # после (раунд 1: s1ch 1/6 красных, s2tn 2/5). Проверка «всё
        # красное» здесь была бы ложной посылкой. Отказ тестировщика
        # задачу не роняет — плечо просто вырождается в сегодняшнее
        # (плечо A), и об этом честно пишется в журнал.
        ...
        while productive < self.max_iter + confirm_rounds:
            ...
            if confirming:
                ...
                if report.get("status") == "dispute":
                    ...
                    # Маркер «сдался» (WAV-011): сам факт сдачи терялся в
                    # косвенных признаках — вопросе dispute в инбоксе и
                    # причине блокировки. Без собственной записи ни журнал,
                    # ни доска не могли ответить, СКОЛЬКО раз и где агенты
                    # сдавались, а это и есть минимальный канал эскалации.
                    self.state.log(
                        "agent_gave_up",
                        task=tid,
                        round=iteration,
