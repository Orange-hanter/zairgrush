...
KIND_RU = {
    ...
    "agent_gave_up": "исполнитель сдался",
}
...
NARRATORS: dict[str, Narrator] = {
    ...
    "futile_exhausted": lambda r: (
        ...
        {"rounds", "causes", "stash"}),
    # Сдача исполнителя — факт, а не мнение: фраза называет раунд и суть
    # (summary — единственное, что гарантировано контрактом отчёта), а qid
    # связывает маркер с открытым вопросом, который сдача породила.
    "agent_gave_up": lambda r: (
        (f"исполнитель сдался в раунде {r.get('round')}: "
         f"{_s(r.get('summary'))} (вопрос {r.get('qid')})"),
        {"round", "summary", "qid"}),
}
