...
- Закрыты все долги, оставленные при исполнении (коммиты
  `d68d75f`…`e4942a6`): исключения квоты дедуплицированы (crash-safety-
  тест пересмотрен по смыслу — защищает детекцию по имени класса);
  флакер `test_all_done_is_zero` устранён в корне (патч не той копии
  `_board_open` + явная `_board_close`); SLF001 снят полностью — общие
  хелперы публичные, ignore-строки удалены, список игноров только
  сократился; `agents.py` доразобран — сборка промптов в
  `promptbuilder.py` (832→541), интерфейс `AgentsLike` Protocol по
  образцу `LoopLike`. Резка `implement`/`review` остаётся отложенной
  по решению §5 — это единственный сознательно оставленный долг.
